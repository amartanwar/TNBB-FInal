...
public class ReceiptPostingDataSetupScript_E2E extends BaseTest {


	public void receiptPostingDataSetupScript(WebDriver driver, ExtentTest test, Map<String, String> map, int i) throws java.text.ParseException 
	{
		...
		if(Integer.parseInt(map.get("Post By Receivable ?"))==1)
		{
			...
			receiptpostingpage.enterCheckAmount(map.get("Check Amount"+i));
			...
			receiptbatchsetuppage.enterBatchName(map.get("Batch Name"+i));
			receiptbatchsetuppage.enterBatchAmount(map.get("Batch Amount"+i));
			...
			while(format.parse(driver.findElement(By.id("ctl00xFCxPHxgrdPaymentxgrdList_rc_"+j+"_7")).getText().toString()).compareTo(format.parse(map.get("Receipt Posting Date"+i)))<=0)
			...
			receiptpostingpage.handleAlert();
			
			receivablesandreceiptmenupage.openReceiptBatchManagement();
			
			receiptbatchlist.enterBatchName(map.get("Batch Name"+i));

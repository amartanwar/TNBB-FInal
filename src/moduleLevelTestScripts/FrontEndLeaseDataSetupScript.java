...
import pom.TransactionSyndicationPage;
...

{
	...
	Takes LOC number as input and creates a TakeDown*/
		...
		{	
			...
			if(Integer.parseInt(map.get("Is Syndicated"))==1)
			{
				transactionhomepage.clickOnBriefLink();
				TransactionBriefPage transactionbriefpage = new TransactionBriefPage(driver, test);
				transactionbriefpage.verifyingSyndicatedCheckBoxAndClicking();  
				//FSN101
				transactionbriefpage.enterFundingSource(map.get("Funding Source"));
				transactionbriefpage.clickOnSaveButton();
			}
			...
			if(Integer.parseInt(map.get("Is Syndicated"))==0)
			{
			...
			if(Integer.parseInt(map.get("Is Syndicated"))==1)
			{
				String leaseTransactionAssetsWindow = transactionleaseassetpage.clickOnImportButton();
				
				AssetListPage assetlistpage = new AssetListPage(driver, test);
				assetlistpage.searchByAlias(map.get("InventoryAlias1"));
				assetlistpage.clickOnSearchButton();
				assetlistpage.clickOnSelectAll();
				assetlistpage.clickOnSelectButton(leaseTransactionAssetsWindow);
				
				transactionleaseassetpage.clickOnPrimaryAsset();
				transactionleaseassetpage.clickSaveAndContinueButton();
				
				if((Integer.parseInt(map.get("Total Number Of Investor Assets")))>1)
				{
					
				
					for(int i =2;i<=(Integer.parseInt(map.get("Total Number Of Investor Assets")));i++)
					{
						String leaseTransactionAssetsWindownew = transactionleaseassetpage.clickOnImportButton();
						assetlistpage.searchByAlias(map.get("InventoryAlias"+i));
						assetlistpage.clickOnSearchButton();
						assetlistpage.clickOnSelectAll();
						assetlistpage.clickOnSelectButton(leaseTransactionAssetsWindownew);
						transactionleaseassetpage.clickSaveAndContinueButton();
					}
				}
			...
			if(Integer.parseInt(map.get("Is Syndicated"))==1)
			{
				transactionhomepage.clickOnSyndicationLink();
				
				TransactionSyndicationPage transactionsyndicationpage = new TransactionSyndicationPage(driver, test);
				transactionsyndicationpage.selectSyndicationGLTemplate(map.get("Syndication GL Template"));
				transactionsyndicationpage.selectLTRToARGLTemplate(map.get("Syndication LTR-AR Template"));
				transactionsyndicationpage.selectReceiptCashGLTemplate(map.get("Syndication Receipt Cash GL Template"));
				transactionsyndicationpage.selectOTPARGLTemplate(map.get("OTP AR GL Template"));
				transactionsyndicationpage.enterSyndicationFee(map.get("Syndication Fee"));
				transactionsyndicationpage.enterFundingDate(map.get("Enter Funding Date"));
				transactionsyndicationpage.enterNetInvestorFundedAmount(map.get("Net Investor Funded Amount"));
				transactionsyndicationpage.clickOnSaveButton();
			}
		...
		//	lcp.selectOTPARGLTemplate(map.get("OTPARGLTemplate"));
			...
			if(Integer.parseInt(map.get("Is Syndicated"))==1)
			{
				//Flow Management - Push TakeDown to S16
				for(int k=1;k<=3;k++)
				{
					transactionhomepage.clickOnOverrideException();
					String beforewindow1 = transactionhomepage.clickOnFlowManagementtLink();
				...
				leaseclassificationpage.enterTotalEconomicLife(map.get("Total Economic Life"));
				leaseclassificationpage.enterTotalRemainingLife(map.get("Total Remaining Life"));
			//	leaseclassificationpage.clickOnOverRideSytemClassification();
				leaseclassificationpage.clickOnPerform90PercentTestButton();
				leaseclassificationpage.handleAlert();
				leaseclassificationpage.handleAlert();
				Thread.sleep(1000);
			...
			//	lcp.selectOTPARGLTemplate(map.get("OTPARGLTemplate"));
				...
				//Flow Management to push TakeDown to Syndication Last Step
				
					transactionhomepage.clickOnOverrideException();
					String beforewindow1 = transactionhomepage.clickOnFlowManagementtLink();
					JarkFlowPage jarkflowpage = new JarkFlowPage(driver, test);
					jarkflowpage.clickOnFlowChange();
					jarkflowpage.clickOnAlternatePath();
					jarkflowpage.selectAlternatePath();
					jarkflowpage.clickOnContinue(beforewindow1);
			...
			//Flow Management - Push TakeDown to S18
			for(int k=1;k<=5;k++)
			{
				transactionhomepage.clickOnOverrideException();
				String beforewindow1 = transactionhomepage.clickOnFlowManagementtLink();

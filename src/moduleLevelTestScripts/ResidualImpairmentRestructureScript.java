...
public class ResidualImpairmentRestructureScript {
	

	public void residualImpairmentRestructureScript(WebDriver driver, ExtentTest test,Map<String, String> map) throws InterruptedException, IOException
	{
		LeaseMenuPage leasemenu = new LeaseMenuPage(driver, test);
		leasemenu.openResidualImpairmentPage();
		...
		if(Integer.parseInt(map.get("Is Residual Entered At Asset Level ?"))==1)
		{
			...
			{
			leaseinventoryinlease.enterResidualBookedAmount(map.get("Residual Booked Amount"+i), map.get("Inventory alias"+ i));
			}
		}
